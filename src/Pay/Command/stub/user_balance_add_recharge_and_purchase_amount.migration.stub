...
class BizUserBalanceAddRechargeAndPurchaseAmount extends Migration
{
    ...
    public function up()
    {
        ...
        if (!$this->isFieldExist('biz_user_balance', 'recharge_amount')) {
            $connection->exec("ALTER TABLE `biz_user_balance` ADD COLUMN `recharge_amount` BIGINT(10) unsigned NOT NULL DEFAULT '0' COMMENT '充值总额'");
        }

        if (!$this->isFieldExist('biz_user_balance', 'purchase_amount')) {
            $connection->exec("ALTER TABLE `biz_user_balance` ADD COLUMN `purchase_amount` BIGINT(10) unsigned NOT NULL DEFAULT '0' COMMENT '消费总额'");
        }
    }
